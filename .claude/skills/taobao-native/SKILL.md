...
- **⚠️ 核心问题**：搜索结果价格 / 页面默认显示价格 ≠ 真实SKU价格
  ...
  - 例如：某Java Neo自行车搜索显示￥135，实际16寸自行车￥2999（￥135是配件价）
- **错误方法（导致价格错误）**：
  ```bash
  # ❌ 错误：直接读取页面价格 → 得到的是默认/起步价
  taobao-native navigate_to_url --args '{"url":"<商详URL>"}'
  taobao-native read_page_content --args '{}'  # 价格不准确！
  ```
- **正确方法（获取真实SKU价格）**：
  1. `taobao-native navigate_to_url --args '{"url":"<商详URL>"}'`：打开商品详情页
  2. `taobao-native get_product_skus --args '{"sourceApp":"copaw"}'`：获取SKU维度和可选规格列表
  3. 根据需求选择目标SKU规格（如"16英寸"、"亮光钛"等）
  4. `taobao-native add_to_cart --args '{"sourceApp":"copaw","sku":["<完整SKU数组>"]}'`：选择目标SKU规格加购
  5. `taobao-native read_page_content --args '{"sourceApp":"copaw"}'`：读取**已选SKU后的页面价格**
  6. 用 `grep -o '￥[0-9]*'` 或正则提取真实价格
- **CLI 调用示例**：
  ```bash
  # 步骤1: 打开商品页
  ...
  # 步骤2: 获取SKU维度（必须先获取）
  taobao-native get_product_skus --args '{"sourceApp":"copaw"}' -o sku.json
  
  # 步骤3: 选择目标SKU加购（SKU数组必须完整，与维度数量匹配）
  taobao-native add_to_cart --args '{"sourceApp":"copaw","sku":["16英寸","亮光钛【下单送豪礼】","16英寸"]}' -o add.json
  
  # 步骤4: 读取已选SKU后的真实价格
  taobao-native read_page_content --args '{"sourceApp":"copaw"}' -o price.json
  
  # 步骤5: 提取价格
  grep -o '￥[0-9]*' price.json | head -1
  ```
- **SKU参数要求**：
  - `sku` 数组长度必须与 `get_product_skus` 返回的维度数量完全匹配
  - 多维度SKU需用 `|` 分隔展示完整规格，如：`尺寸: 16英寸 | 颜色分类: 亮光钛`
  - 单维度SKU（尺寸已包含在规格文本中）直接展示完整文本
  - 部分商品存在"其他"占位维度，实际尺寸在颜色分类中指定，需完整传入
- **关键说明**：
  - **必须先选择SKU再加购，然后读取价格**，否则页面显示的是默认起步价
  - 加购成功后页面价格会更新为目标SKU的真实价格
  - 若加购返回 `needsSkuSelection: true`，说明SKU选择不完整或无货，需调整SKU参数
  - 此方法同样适用于「加购」场景，获取价格只是额外收益
